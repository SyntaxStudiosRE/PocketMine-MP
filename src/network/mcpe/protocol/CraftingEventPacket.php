...
class CraftingEventPacket extends DataPacket implements ServerboundPacket{
	...
	protected function decodePayload(ByteBufferReader $in, int $protocolId) : void{
		...
		for($i = 0; $i < $size and $i < 128; ++$i){
			$this->input[] = $protocolId >= ProtocolInfo::PROTOCOL_1_26_40 ? CommonTypes::getNetworkItemStackDescriptor($in, $protocolId) : CommonTypes::getItemStackWrapper($in, $protocolId);
		}

		$size = VarInt::readUnsignedInt($in);
		for($i = 0; $i < $size and $i < 128; ++$i){
			$this->output[] = $protocolId >= ProtocolInfo::PROTOCOL_1_26_40 ? CommonTypes::getNetworkItemStackDescriptor($in, $protocolId) : CommonTypes::getItemStackWrapper($in, $protocolId);
		}
	}

...
class LevelChunkPacket extends DataPacket implements ClientboundPacket{
	...
	protected function decodePayload(ByteBufferReader $in, int $protocolId) : void{
		...
		if($protocolId >= ProtocolInfo::PROTOCOL_1_26_40){
			$rawSubChunkCount = VarInt::readUnsignedInt($in);
			$this->clientSubChunkRequestsEnabled = CommonTypes::getBool($in);
			$this->subChunkCount = $this->clientSubChunkRequestsEnabled ? VarInt::readUnsignedInt($in) : $rawSubChunkCount;
		}else{
	...
	protected function encodePayload(ByteBufferWriter $out, int $protocolId) : void{
		...
		if($protocolId >= ProtocolInfo::PROTOCOL_1_26_40){
			if($this->clientSubChunkRequestsEnabled){
				...
				VarInt::writeUnsignedInt($out, $this->subChunkCount === PHP_INT_MAX ? Limits::UINT32_MAX : $this->subChunkCount);
			}else{

...
final class PlayerAuthInputFlags{
	...
	/**
	 * @since protocol 1.26.40
	 */
	public const INTERNAL_UPDATE = 65;

	public const NUMBER_OF_FLAGS = 65;
	/**
	 * Protocol >= 1.26.40 adds INTERNAL_UPDATE (index 65), which doesn't fit in the older dense bitset size.
	 * The 2168+ wire format doesn't use a fixed-width bitset at all (it's a sparse list of set indices), so
	 * this only needs to be large enough to hold the highest known flag index.
	 */
